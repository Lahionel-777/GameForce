...
// helper simple para escapar al devolver (HTML)
...
const register = async (req, res, next) => {
    try {
        // Validaciones express-validator (si no usas handleValidationErrors aquí, hazlo)
        ...
        if (!errors.isEmpty()) {
            const seen = new Set();
            const filtered = errors.array().filter(e => {
                if (seen.has(e.param)) return false;
                seen.add(e.param);
                return true;
            }).map(e => ({ campo: e.param, mensaje: e.msg }));
        ...
        // En este punto, req.body ya fue sanitizado por sanitizeInput
        const { firstName, lastName, email, password, phone } = req.body;

        // Aquí: hash de password y creación de usuario (omito hash por brevedad)
        const user = new User({
            firstName: firstName, // limpio y sin tags
            lastName: lastName,
            email,
            password, // recuerda hashear antes de guardar en prod
            phone
        });
        ...
        // Responder con valores escapados para evitar XSS en frontend
